            ...
            {categories.map((item, index) => (
                <TouchableOpacity key={index} onPress={() => handleCategoryPress(item.category)} className={`flex flex-col items-start mr-4 px-4 py-2 rounded-full ${selectedCategory === item.category ? 'bg-primary-300' : 'bg-primary-100 border border-primary-200'}   `}>
                    <Text className={`text-sm ${selectedCategory === item.category ? 'text-white font-rubik-bold mt-0.5' : 'text-black-300 font-rubik'}`}>
                        {item.title === 'All' ? 'Все' :
                         item.title === 'Houses' ? 'Дома' :
                         item.title === 'Condos' ? 'Кондоминиумы' :
                         item.title === 'Duplexes' ? 'Дуплексы' :
                         item.title === 'Studios' ? 'Студии' :
                         item.title === 'Villas' ? 'Виллы' :
                         item.title === 'Apartments' ? 'Квартиры' :
                         item.title === 'Townhouses' ? 'Таунхаусы' :
                         'Остальное'
                        }
                    </Text>
                </TouchableOpacity>
